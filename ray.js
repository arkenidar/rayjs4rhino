...
var RayTracer = (function () {
    ...
    RayTracer.prototype.render = function (scene,canvas) {
        var screenWidth=256, screenHeight=256
...
function draw(canvas){
  ...
  rayTracer.render(defaultScene(),canvas)
}

mainFrame=new java.awt.Frame('JS RayTracing in AWT Canvas with Rhino')
mainFrame.setSize(256,256)

mainFrame.addWindowListener({windowClosing:function(windowEvent){mainFrame.dispose()}})

function CanvasClass(){

this.paint=function paint(g) {
	
	function clampColor(c){
		function clampi(i){
			return Math.min(1.0,i)
		}
		return {
            r: clampi(c.r),
            g: clampi(c.g),
	...
	function setColor(c){
		c=clampColor(c)
		ig.setColor(new java.awt.Color( c.r, c.g, c.b ))
	}
	function drawPoint(x,y){
		ig.fillRect(x,y,x+1,y+1)
	}
	
	g.drawString('rendering in progress...',50,50)
	print('rendering started')

	image=this.createImage(256,256)
	ig=image.getGraphics()
	
	draw({setColor:setColor,drawPoint:drawPoint})
	g.drawImage(image,0,0,this)
	print('rendering done')
	}
}

canvas = new JavaAdapter(java.awt.Canvas,new CanvasClass())
